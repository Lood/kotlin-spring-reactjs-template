...
class CoffeeShopsList extends Component {
    ...
    async componentDidMount() {
        ...
        if (!response.ok) {
            this.setState({
                errorMaessage: `Failed to load coggee shops: ${response.status} ${response.statusText}`,
                isLoading: false
            });
        } else {
            const body = await response.json();
            const coggeeShops = body._embedded.coffeeshops;
            this.setState({
                coffeeShops: coggeeShops,
                isLoading: false,
                errorMessage: null
            });
        }
    ...
    async remove(id) {
        ...
        if (!response.ok) {
            ...
            let updatedCoffeeSops = [...this.state.coffeeShops].filter(i => i.id !== id);
            this.setState({coffeeShops: updatedCoffeeSops, errorMessage: null});
        }
    }
